...
**Reverse Symmetrize**:
The keyframe workflow that mirrors a subtree's keyframes in time about a chosen Center. Position/rotation/scale reflect the delta against the pose both the source and its target hold at that Center; opacity, zIndex, and morph are copied verbatim (`value(t') = value(t)`, tangents mirrored temporally). Invoked from the scene-tree context menu (`Reverse Symmetrize…`) on a subtree whose nodes or their guessed siblings carry any mirrored keyframe; each row picks a symmetrical sibling (guessed by lateral name swap, e.g. Left Ear ↔ Right Ear) or mirrors the object onto itself. Every sibling pair exchanges keys in both directions — selecting either side writes both — so a static object receives its animated sibling's mirrored keys. The modal also picks the mirror axis (X = left↔right, Y = top↔bottom, applied to all rows) and the source interval [From, To] to copy; mirrored times are `2·Center − t`, existing keyframes at those times are overwritten, and mirrored times outside the slide are dropped. Morph keyframes remap their shape pair to symmetrical counterparts, guessed per shape from mirrored geometry first, then the axis-aware name swap (`left`↔`right` on X, `up`↔`down`/`top`↔`bottom` on Y), then absolute copies, and always user-selectable per shape (category shown); a sibling row with an unmapped shape blocks confirm. One undo step; material, clip, and control tracks are untouched.
_Avoid_: Reverse and Save (time-reversal of a clip), Symmetrize (in-place spatial mirroring)
