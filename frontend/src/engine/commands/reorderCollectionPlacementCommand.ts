...
export class ReorderCollectionPlacementCommand implements Command<ReorderCollectionPlacementInverse> {
  ...
  constructor(input: ReorderCollectionPlacementParameters) {
    ...
    if (!Number.isInteger(input.newIndex) || input.newIndex < 0) throw new Error('newIndex must be non-negative integer')
    this.#parentNodeId = input.parentNodeId
    this.#placementId = input.placementId
    this.#newIndex = input.newIndex
    this.parameters = { parentNodeId: input.parentNodeId, placementId: input.placementId, newIndex: input.newIndex }
  }
  ...
  validate(engine: Engine): void {
    ...
    if (this.#newIndex >= parent.collectionPlacements.length) throw new Error(`newIndex out of bounds: ${this.#newIndex}`)
  }

  execute(engine: Engine): ReorderCollectionPlacementInverse {
    const oldIndex = engine.reorderCollectionPlacement(this.#parentNodeId, this.#placementId, this.#newIndex)
