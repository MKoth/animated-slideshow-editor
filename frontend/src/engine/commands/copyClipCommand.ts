...
export interface CopyClipParameters {
...
export interface CopyClipInverse {
...
export class CopyClipCommand implements Command<CopyClipInverse> {
  readonly type = 'CopyClip'
  ...
  constructor(input: CopyClipParameters) {
  ...
  execute(engine: Engine): CopyClipInverse {
    const copied = engine.createCopiedClip(this.#sourceClipId, this.#newName)
    const snapshot = copied.toJSON()
    let instanceId: string | undefined
    let nodeId: string | undefined
    let instanceSnapshot: unknown | undefined
    if (this.#targetNodeId !== undefined) {
      const start = this.#startTime ?? 0
      const inst = engine.assignClipInstance(this.#targetNodeId, copied.id, start, 1, true, {})
      instanceId = inst.id
