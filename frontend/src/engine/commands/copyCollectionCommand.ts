...
export interface CopyCollectionParameters {
...
export interface CopyCollectionInverse {
...
export class CopyCollectionCommand implements Command<CopyCollectionInverse> {
  readonly type = 'CopyCollection'
  ...
  constructor(input: CopyCollectionParameters) {
  ...
  execute(engine: Engine): CopyCollectionInverse {
    const { collection, clipIdMap } = engine.createCopiedCollection(
