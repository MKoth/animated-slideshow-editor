...
export function rebuildTableChild(
  ...
  const child = node.components.tableCell ? createTableCellContainer(pixi, node) : undefined
  if (!child) return undefined
  for (const visual of [...container.children]) {
    if (visual.label.startsWith('table-cell:')) {
      visual.destroy()
    }
  }
...
export function rebuildTableChildWithEvaluated(
  ...
  const child = node.components.tableCell
