...
import { PlaceCollectionCommand, ReorderCollectionPlacementCommand, MoveClipLayerCommand } from '../engine/commands'
import { SetClipInstanceSpeedCommand, SetClipInstanceStartTimeCommand, TransactionCommand, SetCollectionPlacementStartTimeCommand } from '../engine/commands'
...
import { packCollectionLanesForParent, visualDurationForCollectionPlacement } from '../engine/animationManagerModel'
import { noopPersistence } from './contextHarness'

function renderManager(engine: Engine, undo: UndoStack, parentNodeId: string | null, onClose = () => {}) {
...
describe('Animation Manager 15-06 – Collection Lane single bar, uniform stretch, move and stacking with two-axis reorder', () => {
  it('Collection Lane renders as single bar on parent top collection section, hidden internals, tooltip', async () => {
    ...
    const dispatcher = new CommandDispatcher(engine, undo, () => undefined)
    const clip1 = (dispatcher.dispatch(new CreateClipCommand({ name: 'ClipA', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    const clip2 = (dispatcher.dispatch(new CreateClipCommand({ name: 'ClipB', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    ...
    const placeRes = dispatcher.dispatch(new PlaceCollectionCommand({ collectionId: col.id, parentNodeId: parent.id, startTime: 0 })) as any
  ...
  it('Uniform stretch via speed factor newSpeed = oldSpeed * oldVisual / newVisual in one Transaction; MIN_VISUAL 0.25', async () => {
    ...
    const dispatcher = new CommandDispatcher(engine, undo, () => undefined)
    const clip1 = (dispatcher.dispatch(new CreateClipCommand({ name: 'C1', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    const clip2 = (dispatcher.dispatch(new CreateClipCommand({ name: 'C2', duration: 4, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    ...
    const getClip = (id: string) => { try { return engine.getClip(id) } catch { return null } }
    ...
    for (const m of members) {
      ...
      cmds.push(new SetClipInstanceSpeedCommand({ nodeId: m.nodeId, instanceId: m.instance.id, speed: newSpeed }))
    }
  ...
  it('Move shifts all member startTimes together; v1 zero-offset', async () => {
    ...
    const dispatcher = new CommandDispatcher(engine, undo, () => undefined)
    const clip1 = (dispatcher.dispatch(new CreateClipCommand({ name: 'C1', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    const clip2 = (dispatcher.dispatch(new CreateClipCommand({ name: 'C2', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    ...
    for (const m of membersBefore) {
      cmds.push(new SetClipInstanceStartTimeCommand({ nodeId: m.nodeId, instanceId: m.instance.id, startTime: m.instance.startTime + delta }))
    }
    const tx = new TransactionCommand(cmds)
    ...
    const getClip = (id: string) => { try { return engine.getClip(id) } catch { return null } }
    const vd = visualDurationForCollectionPlacement(engine.getCollectionPlacement(placementId), parent, getClip)
  ...
  it('Vertically overlapping collection lanes stack greedily; lower wins Priority, unified with clip stacking', async () => {
    ...
    c1.channelAnimation('positionX' as any)?.add({ id: 'k1', time: 0, value: 0, interpolation: 'linear', tangentIn: { time: 0, value: 0 }, tangentOut: { time: 0, value: 0 } } as any)
    c1.channelAnimation('positionX' as any)?.add({ id: 'k2', time: 1, value: 100, interpolation: 'linear', tangentIn: { time: 0, value: 0 }, tangentOut: { time: 0, value: 0 } } as any)
    const c2 = engine.getClip(clip2)
    c2.channelAnimation('positionX' as any)?.add({ id: 'k3', time: 0, value: 0, interpolation: 'linear', tangentIn: { time: 0, value: 0 }, tangentOut: { time: 0, value: 0 } } as any)
    c2.channelAnimation('positionX' as any)?.add({ id: 'k4', time: 1, value: 999, interpolation: 'linear', tangentIn: { time: 0, value: 0 }, tangentOut: { time: 0, value: 0 } } as any)
    ...
    dispatcher.dispatch(new PlaceCollectionCommand({ collectionId: col1.id, parentNodeId: parent.id, startTime: 0 }))
    dispatcher.dispatch(new PlaceCollectionCommand({ collectionId: col2.id, parentNodeId: parent.id, startTime: 1 }))
    const getClip = (id: string) => { try { return engine.getClip(id) } catch { return null } }
    const getCollection = (id: string) => { try { return engine.getClipCollection(id) } catch { return null } }
  ...
  it('Two-axis drag reorders: horizontal → startTime, vertical → order; persists node.clipInstances and collectionPlacements; reorder is one Transaction', async () => {
    ...
    engine.setSemanticName(child.id, 'hand')
    const dispatcher = new CommandDispatcher(engine, undo, () => undefined)
    const clipA = (dispatcher.dispatch(new CreateClipCommand({ name: 'A', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    const clipB = (dispatcher.dispatch(new CreateClipCommand({ name: 'B', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    engine.getClip(clipA).channelAnimation('positionX' as any)?.add({ id: 'ka1', time: 0, value: 0, interpolation: 'linear', tangentIn: { time: 0, value: 0 }, tangentOut: { time: 0, value: 0 } } as any)
    engine.getClip(clipA).channelAnimation('positionX' as any)?.add({ id: 'ka2', time: 1, value: 10, interpolation: 'linear', tangentIn: { time: 0, value: 0 }, tangentOut: { time: 0, value: 0 } } as any)
    engine.getClip(clipB).channelAnimation('positionX' as any)?.add({ id: 'kb1', time: 0, value: 0, interpolation: 'linear', tangentIn: { time: 0, value: 0 }, tangentOut: { time: 0, value: 0 } } as any)
    engine.getClip(clipB).channelAnimation('positionX' as any)?.add({ id: 'kb2', time: 1, value: 100, interpolation: 'linear', tangentIn: { time: 0, value: 0 }, tangentOut: { time: 0, value: 0 } } as any)
    ...
    const pARes = dispatcher.dispatch(new PlaceCollectionCommand({ collectionId: colA.id, parentNodeId: parent.id, startTime: 0 })) as any
    const pBRes = dispatcher.dispatch(new PlaceCollectionCommand({ collectionId: colB.id, parentNodeId: parent.id, startTime: 0 })) as any
    ...
    let evalBefore = engine.evaluateNode(child.id, 0.5)
    ...
    const beforeLen = undo.entries.length
    const reorderRes = dispatcher.dispatch(new ReorderCollectionPlacementCommand({ parentNodeId: parent.id, placementId: pAId, newIndex: 1 })) as any
    expect(reorderRes.ok).toBe(true)
    expect(undo.entries.length).toBe(beforeLen + 1) // one Transaction
    // After reorder, order [B,A], A is lower and should win
    let evalAfter = engine.evaluateNode(child.id, 0.5)
    ...
    const clipC = (dispatcher.dispatch(new CreateClipCommand({ name: 'C', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    const clipD = (dispatcher.dispatch(new CreateClipCommand({ name: 'D', duration: 7, category: '', params: [], channels: [{ property: 'positionX' }] })) as any).inverse.clipId as string
    ...
    const moveRes = dispatcher.dispatch(new MoveClipLayerCommand({ nodeId: child.id, instanceId: instD.id, newIndex: 0 })) as any
    expect(moveRes.ok).toBe(true)
    expect(child.clipInstances[0]!.id).toBe(instD.id)
    // Horizontal move would be SetClipInstanceStartTime, also one Transaction
    const horizRes = dispatcher.dispatch(new SetClipInstanceStartTimeCommand({ nodeId: child.id, instanceId: instC.id, startTime: 5 })) as any
