...
function makeEngineWithClip(opts: {
  ...
  return { engine, readOnly, dispatcher, undo, slideId: slide.id, clipId: clip.id, assetId: 'audio-1', clip, peaks }
}
...
class MockAudio {
  ...
  static clear() { MockAudio.instances = [] }
}
...
beforeEach(() => {
  ...
  vi.stubGlobal('fetch', vi.fn(async () => new Response(JSON.stringify({ peaks: null }), { status: 200 })))
})

describe('Waveform Editor — #265', () => {
  it('Modal renders waveform via cached waveformPeaks / decoded peaks', async () => {
    const { readOnly, dispatcher, slideId, clipId } = makeEngineWithClip({ assetDuration: 8, waveformPeaks: Array(800).fill(200) })
    const dispatch = (cmd: Parameters<CommandDispatcher['dispatch']>[0]) => dispatcher.dispatch(cmd)
    render(
      <EngineContext.Provider value={{ engine: readOnly, dispatch: dispatch as never, undoStack: new UndoStack(), persistence: noopPersistence }}>
  ...
  it('Decoded peaks fallback when cached missing and duration <30', async () => {
    ...
    w(0,'RIFF'); view.setUint32(4,36+dataSize,true); w(8,'WAVE'); w(12,'fmt '); view.setUint32(16,16,true); view.setUint16(20,1,true); view.setUint16(22,1,true); view.setUint32(24,sampleRate,true); view.setUint32(28,byteRate,true); view.setUint16(32,2,true); view.setUint16(34,16,true); w(36,'data'); view.setUint32(40,dataSize,true)
    const bytes = new Uint8Array(buffer)
    let bin='' ; for(let i=0;i<bytes.length;i++) bin+=String.fromCharCode(bytes[i])
    const b64 = btoa(bin)
    engine.embedAsset({ id:'audio-decode', name:'dec.wav', data:b64, mimeType:'audio/wav', metadata:{ duration:1, sampleRate:8000, channels:1 } })
    const clip = engine.createAudioClip(slide.id, { assetId:'audio-decode', trackId:'voice', timelineStart:0, sourceStart:0, sourceEnd:1 })
    const readOnly = toReadOnly(engine)
    // Mock AudioContext to produce decoded peaks
    const mockBuffer = { duration:1, sampleRate:8000, numberOfChannels:1, length:8000, getChannelData:()=>new Float32Array(8000).fill(0.3) } as unknown as AudioBuffer
    const Ctor = vi.fn().mockImplementation(()=>({ decodeAudioData: vi.fn().mockResolvedValue(mockBuffer), close: vi.fn().mockResolvedValue(undefined)}))
    vi.stubGlobal('AudioContext', Ctor as unknown as typeof AudioContext)
    ;(window as unknown as {AudioContext?: unknown}).AudioContext = Ctor as unknown as typeof AudioContext
    const dispatch = (cmd: Parameters<CommandDispatcher['dispatch']>[0]) => dispatcher.dispatch(cmd)
    render(
      <EngineContext.Provider value={{ engine: readOnly, dispatch: dispatch as never, undoStack: new UndoStack(), persistence: noopPersistence }}>
  ...
  it('Edge trim updates sourceStart/End; Save as single Transaction and undo restores; no asset rewrite', async () => {
    ...
    const { engine, readOnly, dispatcher, undo, slideId, clipId } = makeEngineWithClip({ assetDuration: 10, sourceStart: 0, sourceEnd: 10, timelineStart: 2, waveformPeaks: Array(800).fill(100), assetBase64: origData })
    const dispatch = (cmd: Parameters<CommandDispatcher['dispatch']>[0]) => dispatcher.dispatch(cmd)
    const onClose = vi.fn()
    render(
      <EngineContext.Provider value={{ engine: readOnly, dispatch: dispatch as never, undoStack: undo, persistence: noopPersistence }}>
  ...
  it('Middle delete splits into two clips gap-free; timeline reflects new clips; undo single Transaction', async () => {
    const { engine, readOnly, dispatcher, undo, slideId, clipId } = makeEngineWithClip({ assetDuration: 10, sourceStart: 0, sourceEnd: 10, timelineStart: 5, waveformPeaks: Array(800).fill(100) })
    const dispatch = (cmd: Parameters<CommandDispatcher['dispatch']>[0]) => dispatcher.dispatch(cmd)
    const onClose = vi.fn()
    render(
      <EngineContext.Provider value={{ engine: readOnly, dispatch: dispatch as never, undoStack: undo, persistence: noopPersistence }}>
    ...
    vi.spyOn(container, 'getBoundingClientRect').mockReturnValue({ left: 0, top: 0, width: 640, height: 96, right: 640, bottom: 96, x:0, y:0, toJSON:()=>{} } as DOMRect)
  ...
  it('Audition plays selected interval; respects volume/muted and playbackRate', async () => {
    const { readOnly, dispatcher, slideId, clipId } = makeEngineWithClip({ assetDuration: 6, sourceStart: 0, sourceEnd: 6, volume: 0.6, muted: false, waveformPeaks: Array(800).fill(100) })
    const dispatch = (cmd: Parameters<CommandDispatcher['dispatch']>[0]) => dispatcher.dispatch(cmd)
    MockAudio.clear()
    render(
      <EngineContext.Provider value={{ engine: readOnly, dispatch: dispatch as never, undoStack: new UndoStack(), persistence: noopPersistence }}>
    ...
    eng2.embedAsset({ id:'a-muted', name:'m.wav', data:'dGVzdA==', mimeType:'audio/wav', metadata:{ duration:4, sampleRate:44100, channels:1, waveformPeaks:Array(800).fill(80)}})
    const cl2 = eng2.createAudioClip(sl2.id, { assetId:'a-muted', trackId:'voice', timelineStart:0, sourceStart:0, sourceEnd:4, volume:0.9, muted:true })
    const ro2b = toReadOnly(eng2)
    const dispatch2 = (cmd: Parameters<CommandDispatcher['dispatch']>[0]) => disp2.dispatch(cmd)
    const { unmount } = render(
      <EngineContext.Provider value={{ engine: ro2b, dispatch: dispatch2 as never, undoStack: new UndoStack(), persistence: noopPersistence }}>
  ...
  it('Gap-free reflow after trim edge — timeline reflects new playback length and second clip not shifted (only split reflows)', async () => {
    const { engine, readOnly, dispatcher, slideId, clipId } = makeEngineWithClip({ assetDuration: 10, sourceStart: 2, sourceEnd: 8, timelineStart: 0 })
    // Add downstream clip at 8
    engine.createAudioClip(slideId, { assetId:'audio-1', trackId:'voice', timelineStart: 8, sourceStart:0, sourceEnd:2 })
    const dispatch = (cmd: Parameters<CommandDispatcher['dispatch']>[0]) => dispatcher.dispatch(cmd)
