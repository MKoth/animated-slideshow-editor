...
describe('Shadow CastShadow seam #300', () => {
  it('getCastShadow returns false for Bone/Ghost/Camera, else n.castShadow ?? true, pruning', () => {
    ...
    const bone = engine.createNode(slide.scene.id, group.id, 'Bone', {
      components: { bone: { kind: 'bone', length: 20 } },
    })
    const ghost = engine.createNode(slide.scene.id, group.id, 'Ghost', {
      components: { ghost: { kind: 'ghost' } },
    })
    const asset = engine.createNode(slide.scene.id, group.id, 'Asset', {
      components: { assetInstance: { kind: 'assetInstance', assetDefinitionId: 'def1' } },
    })
    const text = engine.createNode(slide.scene.id, group.id, 'Text', {
      components: { text: { kind: 'text', content: 'hi', fontSize: 12, alignment: 'left' } },
    })
  ...
  it('Cast Shadow = false prunes entire subtree — child cannot re-enable', () => {
    ...
    const childAsset = engine.createNode(slide.scene.id, limb.id, 'ChildAsset', {
      components: { assetInstance: { kind: 'assetInstance', assetDefinitionId: 'def1' } },
    })
    // Need circle or mesh for renderable check — assetInstance is renderable
    engine.createNode(slide.scene.id, host.id, 'Accessory', {
    ...
    let casters = collectShadowCasters(
  ...
  it('Bone prunes, placeholder casts', () => {
    ...
    const boneGroup = engine.createNode(slide.scene.id, host.id, 'BoneGroup', {
      components: { bone: { kind: 'bone', length: 30 } },
    })
    const boneChild = engine.createNode(slide.scene.id, boneGroup.id, 'BoneChild', {
      components: { assetInstance: { kind: 'assetInstance', assetDefinitionId: 'defX' } },
    })
    const placeholder = engine.createNode(slide.scene.id, host.id, 'Placeholder', {
      components: { assetInstance: { kind: 'assetInstance', assetDefinitionId: 'missingId' } },
    })
  ...
  it('Nested group with its own Shadow Effect is still included in its parent Silhouette', () => {
    ...
    const childAsset = engine.createNode(slide.scene.id, childGroup.id, 'ChildAsset', {
      components: {
        mesh: {
          kind: 'mesh',
          mesh: {
    ...
    dispatcher.dispatch(
      new SetShadowEffectCommand({ nodeId: childGroup.id, shadowEffect: DEFAULT_SHADOW_EFFECT }),
    )
    ...
    const casters = collectShadowCasters(
      parentGroup as unknown as { children: readonly unknown[] },
    ) as SceneNode[]
  ...
  it('Silhouette covers all renderable kinds', () => {
    ...
    nodes.push(
      engine.createNode(slide.scene.id, host.id, 'Asset', {
        components: { assetInstance: { kind: 'assetInstance', assetDefinitionId: 'a' } },
      }),
    )
    nodes.push(
      engine.createNode(slide.scene.id, host.id, 'Text', {
        components: { text: { kind: 'text', content: 'hi', fontSize: 12, alignment: 'left' } },
      }),
    )
    nodes.push(
      engine.createNode(slide.scene.id, host.id, 'Mesh', {
        components: {
          mesh: {
            kind: 'mesh',
            mesh: {
    ...
    nodes.push(
      engine.createNode(slide.scene.id, host.id, 'Circle', {
        components: { circle: { kind: 'circle', radius: 5, startAngle: 0, endAngle: 360 } },
      }),
    )
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    nodes.push(
      engine.createNode(slide.scene.id, host.id, 'Table', {
        components: {
          table: {
            kind: 'table',
            columns: [{ width: 100 }],
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    nodes.push(
      engine.createNode(slide.scene.id, host.id, 'Row', {
        components: { tableRow: { kind: 'tableRow' } } as any,
      }),
    )
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    nodes.push(
      engine.createNode(slide.scene.id, host.id, 'Cell', {
        components: { tableCell: { kind: 'tableCell', colSpan: 1, rowSpan: 1 } } as any,
      }),
    )
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    nodes.push(
      engine.createNode(slide.scene.id, host.id, 'Chart', {
        components: {
          chart: {
            kind: 'chart',
            chartType: 'bar',
    ...
    engine.createNode(slide.scene.id, group.id, 'Dummy', {
      components: { assetInstance: { kind: 'assetInstance', assetDefinitionId: 'a2' } },
    })
    // EmptyGroup is a group node (no components, has child), should not be caster itself but its child should
    const casters2 = collectShadowCasters(
      host as unknown as { children: readonly unknown[] },
    ) as SceneNode[]
  ...
  it('Command SetCastShadow with one HistoryEntry and JSON round-trip', () => {
    ...
    const slide = engine.getSlide(engine.activeSlideId!)
    const host = engine.createNode(slide.scene.id, slide.scene.root.id, 'Host')
    const asset = engine.createNode(slide.scene.id, host.id, 'Asset', {
      components: { circle: { kind: 'circle', radius: 5, startAngle: 0, endAngle: 360 } },
    })
