...
export function RecordModal({
  ...
  const getUserMediaFn = injectedGetUserMedia ?? ((c: MediaStreamConstraints) => navigator.mediaDevices.getUserMedia(c))
  const RecorderCtor = (InjectedRecorder as unknown as typeof MediaRecorder) ?? (globalThis as unknown as { MediaRecorder?: typeof MediaRecorder }).MediaRecorder
  const AudioCtor = (InjectedAudioCtx as unknown as typeof AudioContext) ?? (globalThis as unknown as { AudioContext?: typeof AudioContext; webkitAudioContext?: typeof AudioContext }).AudioContext ?? (globalThis as unknown as { webkitAudioContext?: typeof AudioContext }).webkitAudioContext
  ...
  const commitRecording = useCallback(
    async (
      ...
      if (part?.audioClipId) {
        try { dispatch(new DeleteAudioClipCommand({ slideId, clipId: part.audioClipId })) } catch { /* ignore */ }
      }
      ...
      if (!assetResult.ok) {
        setErrorInfo({ kind: 'unknown', message: assetResult.error.message, hint: 'Failed to create asset', retryable: true })
        setStatus('error')
        return
      }
      ...
      if (!clipResult.ok) {
        setErrorInfo({ kind: 'unknown', message: clipResult.error.message, hint: 'Failed to create clip', retryable: true })
        setStatus('error')
        return
      }
      ...
      if (!linkResult.ok) {
        setErrorInfo({ kind: 'unknown', message: linkResult.error.message, hint: 'Failed to link', retryable: true })
        setStatus('error')
        return
      }
      onClose()
  ...
  const handleStop = useCallback(async () => {
    ...
    const blob = blobsRef.current.length > 0 ? new Blob(blobsRef.current, { type: (recorderRef.current as unknown as { mimeType?: string })?.mimeType || 'audio/webm' }) : null
    if (!blob || blob.size === 0) {
      setErrorInfo({ kind: 'unknown', message: 'No audio captured.', hint: 'Try recording again.', retryable: true })
      setStatus('error')
      cleanup()
      return
    }
    ...
    try {
      ...
      const metadata = await decodeAudioMetadata(base64, InjectedAudioCtx ? () => new InjectedAudioCtx() : undefined)
  ...
  }, [plannedDuration, InjectedAudioCtx, engine.project, cleanup, commitRecording])
    ...
    try {
      ...
      if (AudioCtor) {
        try {
          ...
          rafRef.current = requestAnimationFrame(tick)
        } catch { /* ignore analyser failure */ }
      }
      if (!RecorderCtor) throw new Error('MediaRecorder not available')
      const recorder = new RecorderCtor(stream, { mimeType: 'audio/webm' } as unknown as MediaRecorderOptions)
  ...
  const stopRecording = useCallback(() => {
    ...
    if (rec && (rec as unknown as { state?: string }).state === 'recording') {
      try { rec.stop() } catch { void handleStop() }
  ...
  const handleMismatchChoice = useCallback(
    async (choice: 'speed' | 'extend' | 'keep' | 'discard' | 'slow', shift: boolean) => {
      ...
      if (choice === 'extend' || choice === 'keep') {
        ...
        const clipId = (clipResult.inverse as { clipId: string }).clipId
        dispatch(new SetPrompterPartAudioCommand({ slideId, partId, audioClipId: clipId, audioAssetId: assetId }))
        const shiftResult = dispatch(
          new UpdatePrompterPartWithShiftCommand({ slideId, partId, duration: recordedDuration, shiftDownstream: shift }),
        )
        if (!shiftResult.ok) {
          setErrorInfo({ kind: 'unknown', message: shiftResult.error.message, hint: 'Failed to update duration', retryable: true })
          setShowMismatch(null)
          return
        }
        setShowMismatch(null)
    ...
    [showMismatch, plannedDuration, commitRecording, dispatch, slideId, partId, partText, partStartTime, part, cleanup, onClose],
  )
  ...
  useEffect(() => {
    const esc = (e: KeyboardEvent) => { if (e.key === 'Escape') { cleanup(); onClose() } }
    window.addEventListener('keydown', esc)
    return () => window.removeEventListener('keydown', esc)
  }, [cleanup, onClose])

  if (showMismatch) {
    const kind = getMismatchKind(showMismatch.recordedDuration, plannedDuration, getPrompterMismatchThreshold(engine.project?.settings ?? {}))
    return (
      <MismatchDialog
        ...
        onClose={() => { setShowMismatch(null); cleanup(); onClose() }}
      />
    )
  }

  return (
    <div role="dialog" aria-modal="true" aria-label="Record" data-testid="record-modal" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }} onClick={(e) => { if (e.target === e.currentTarget) { cleanup(); onClose() } }}>
      <div style={{ background: '#2a2a2a', border: '1px solid #444', borderRadius: 8, width: 460, padding: 16, color: '#e0e0e0' }}>
        ...
        <p style={{ fontSize: 11, color: '#888', margin: '0 0 8px' }}>Part {partStartTime.toFixed(1)}–{(partStartTime + plannedDuration).toFixed(1)} · Voice track · Shortcut <code style={{ background: '#333', padding: '1px 4px', borderRadius: 3 }}>{shortcut.toUpperCase()}</code></p>
        ...
        <div data-testid="record-meter" aria-label="Level meter" style={{ height: 48, background: '#111', borderRadius: 6, display: 'flex', alignItems: 'center', gap: 2, padding: '0 8px', margin: '8px 0', overflow: 'hidden' }}>
          {Array.from({ length: 24 }).map((_, i) => {
            const active = status === 'recording' && level * 24 > i
            const jitter = ((i * 7) % 6)
            return <div key={i} style={{ flex: 1, height: active ? `${6 + level * 32 + jitter}px` : '4px', background: active ? (level > 0.7 ? '#ff4d4d' : level > 0.4 ? '#7c5cff' : '#333') : '#222', borderRadius: 2, transition: 'height 0.08s, background 0.08s' }} />
          })}
        </div>
        ...
        {errorInfo && (
          <div data-testid="record-error" style={{ marginTop: 8, padding: 8, background: '#3a1a1a', border: '1px solid #5a2222', borderRadius: 4, fontSize: 11 }}>
            ...
            {(errorInfo.kind === 'notAllowed' || errorInfo.kind === 'notFound') && (
              <div style={{ marginTop: 6, display: 'flex', gap: 8 }}>
                <button data-testid="record-retry" onClick={() => void startRecording()} style={{ padding: '4px 8px', background: '#7c5cff', color: '#fff', border: 'none', borderRadius: 4, cursor: 'pointer' }}>Retry</button>
                {errorInfo.kind === 'notAllowed' && <span style={{ fontSize: 10, color: '#aaa' }}>Check system settings → Privacy → Microphone</span>}
              </div>
            )}
          </div>
        )}
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12 }}>
          <button data-testid="record-cancel" onClick={() => { cleanup(); onClose() }} style={{ padding: '6px 12px', borderRadius: 4, border: '1px solid #444', background: '#333', color: '#e0e0e0', cursor: 'pointer' }}>Cancel</button>
          {status === 'idle' || status === 'error' ? (
            <button data-testid="record-start" onClick={() => void startRecording()} style={{ padding: '6px 12px', borderRadius: 4, border: '1px solid #7c5cff', background: '#7c5cff', color: '#fff', cursor: 'pointer' }}>Start (R)</button>
          ) : status === 'recording' ? (
            <button data-testid="record-stop" onClick={stopRecording} style={{ padding: '6px 12px', borderRadius: 4, border: '1px solid #c0392b', background: '#c0392b', color: '#fff', cursor: 'pointer' }}>■ Stop</button>
          ) : (
            <button disabled style={{ padding: '6px 12px', borderRadius: 4, border: '1px solid #444', background: '#333', color: '#888' }}>Processing…</button>
          )}
        </div>
      </div>
    </div>
