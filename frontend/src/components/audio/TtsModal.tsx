...
export function TtsModal({
  ...
  // No language filtering — speakers and languages are independent; any speaker can speak any language.
  ...
  // Mode-aware enablement: custom_voice → voice dropdown active, reference disabled
  // voice_clone (Base) → reference active, voice disabled
  // voice_design → both disabled (uses instruction as voice design)
  ...
  // Audio asset reference options: global imported + embedded audio assets
  ...
  useEffect(() => {
    // Ensure asset library loaded for reference dropdown when needed
  ...
  // When ttsModel changes, validate voice against model speakers (no language filtering)
  // Any speaker can speak any language; nativeLanguage is just a hint.
  useEffect(() => {
    if (!voice) return
    if (!isTtsVoiceEnabled) {
      // Voice not applicable for this mode (e.g., Base voice_clone) — clear voice
      setVoice('')
  ...
  const openEdit = () => {
    ...
    // Reference asset for Base (stored in params.referenceAssetId if present)
    ...
    // Init params sliders from stored params
  ...
  // When formModel changes, validate formVoice against model speakers (no language filtering)
  useEffect(() => {
  ...
  const handleAdvancedJsonChange = (val: string) => {
    ...
    try {
      ...
      // keep sliders as is, validation will show error on save
    }
  }
  ...
  const handleSavePrompt = async () => {
    ...
    // Merge slider TTS params (sliders override advanced JSON TTS keys)
    ...
    // Merge voice reference for Base models into params.referenceAssetId
        ...
        >
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <label style={{ fontSize: 11, opacity: isFormVoiceEnabled ? 1 : 0.5 }}>
              ...
              {formSpeakers.length === 0 && isFormVoiceEnabled && (
      ...
      >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
          ...
          <label style={{ fontSize: 11, opacity: isTtsVoiceEnabled ? 1 : 0.5 }}>
            ...
            <div style={{ fontSize: 10, color: '#888', marginTop: 2 }}>
              {isTtsVoiceEnabled ? '9 canonical voices — any language (native is hint only)' : 'Voice selection not applicable for this model'}
            </div>
        ...
        <div style={{ marginBottom: 8 }}>
          <label style={{ fontSize: 11, opacity: isTtsReferenceEnabled ? 1 : 0.5 }}>
            ...
            <div style={{ fontSize: 10, color: '#888', marginTop: 2 }}>
              {isTtsReferenceEnabled ? 'Select voice reference audio from assets (Base clones this voice)' : 'Reference audio only for Base voice_clone models'}
            </div>
          </label>
        </div>
        <div style={{ marginBottom: 12 }}>
